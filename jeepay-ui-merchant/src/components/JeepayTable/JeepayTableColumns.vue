  ...
  export default {
    setup(props, { slots, attrs, emit }) {
      ...
      return () => {
        ...
        if (filterSlots.map(v=>v.children).flat().length <= 2 ) { // 小于等于三个直接渲染
          return h(
            'div',
          ...
          return <div>
               {[slots.default().filter(filterFunc)[0]]}
                <JeeMenu>
                  {
                    slots.default().filter(filterFunc).filter((r, index) => index >= 1).map((item:any) => {
                      if (item.type == 'dr-children' && item.children.length) {
                          const title = item.props.title
                        return <JeepayDrChildren title={title}>{item.children.filter(r => r.props).map(v=>(<MenuItem>{v}</MenuItem>))}</JeepayDrChildren>
                      }else{
