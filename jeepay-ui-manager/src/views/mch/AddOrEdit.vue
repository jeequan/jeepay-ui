      ...
      <a-row justify="space-between" type="flex">
        <a-col :span="10" style="position:relative">
          <a-form-item label="商户类型" name="type">
            <!-- 商户类型 气泡弹窗 -->
            <a-radio-group v-model:value="vdata.saveObject.type" :disabled="vdata.isAdd?false:true">
              <a-radio :value="1">
                普通商户
              </a-radio>
              <a-radio :value="2">
        ...
        <a-col v-if="vdata.saveObject.type == 2" :span="10">
          <a-form-item label="服务商号" name="isvNo">
            <a-select v-model:value="vdata.saveObject.isvNo" placeholder="请选择服务商">
...
const checkIsvNo = (rule, value) => { // 校验类型为特约商户是否选择了服务商
